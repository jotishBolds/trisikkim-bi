...
export default function DignitariesAdmin() {
  ...
  const fetch_ = useCallback(async () => {
    try {
      const r = await fetch("/api/dignitaries");
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const r = await fetch(`/api/dignitaries/${id}`, { method: "DELETE" });
      const d = await r.json();
      if (d.success) fetch_();
      else setError(d.error);
    } catch {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end">
        ...
        >
          <Plus className="w-3.5 h-3.5" /> Add Dignitary
        </Button>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-[#1077a6] text-white">
              <th className="px-3 py-2.5 text-left font-semibold">Photo</th>
            ...
            {items.map((item, i) => (
              ...
              >
                <td className="px-3 py-2.5">
                  {item.image ? (
                    <div className="relative w-8 h-8 rounded-full overflow-hidden bg-[#1077a6]/[0.05]">
                      <Image
                        src={item.image}
                        alt={item.name}
                        fill
                    ...
                    <div className="w-8 h-8 rounded-full bg-[#1077a6]/10 flex items-center justify-center text-[9px] font-bold text-[#1077a6]">
                      {item.name.charAt(0)}
                    </div>
                  )}
                </td>
